...
import { ACCOUNTING_DISTRIBUTION_EMAIL_KEY, ApiError, getSetting, putSetting } from './settingsApi'
...
describe('base URL wiring', () => {
  it('getSetting() builds its request URL from getRefundApiBaseUrl(), not import.meta.env', async () => {
    ...
    const { url } = lastCall()
    expect(url).toBe(`${REFUND_API_URL}/settings/${ACCOUNTING_DISTRIBUTION_EMAIL_KEY}`)
  })
...
describe('getSetting()', () => {
  ...
  it('throws ApiError on 403 (missing settings:read, AC-3.1)', async () => {
    vi.mocked(apiFetch).mockResolvedValueOnce(problemResponse(403, 'Forbidden', 'Missing capability: settings:read'))

    const err = await expectApiError(() => getSetting(ACCOUNTING_DISTRIBUTION_EMAIL_KEY), 403)
    expect(err.detail).toBe('Missing capability: settings:read')
  })

  it('throws ApiError on 404 (unknown key)', async () => {
    vi.mocked(apiFetch).mockResolvedValueOnce(problemResponse(404, 'Not Found', 'Unknown setting key'))
  ...
  it('throws ApiError on 401 (no session)', async () => {
    ...
    await expectApiError(() => getSetting(ACCOUNTING_DISTRIBUTION_EMAIL_KEY), 401)
  })
})
...
describe('putSetting()', () => {
  it('issues PUT /settings/:key with { value } and returns the post-write state (AC-1.2)', async () => {
    ...
    const result = await putSetting(ACCOUNTING_DISTRIBUTION_EMAIL_KEY, 'accounting@welld.ch')
  ...
  it('throws ApiError on 422 for a malformed email (AC-1.3)', async () => {
    vi.mocked(apiFetch).mockResolvedValueOnce(
      problemResponse(422, 'Unprocessable Entity', 'value must be a well-formed email address'),
    )

    const err = await expectApiError(() => putSetting(ACCOUNTING_DISTRIBUTION_EMAIL_KEY, 'not-an-email'), 422)
  ...
  it('throws ApiError on 403 (missing settings:manage, AC-3.1)', async () => {
    vi.mocked(apiFetch).mockResolvedValueOnce(problemResponse(403, 'Forbidden', 'Missing capability: settings:manage'))
    await expectApiError(() => putSetting(ACCOUNTING_DISTRIBUTION_EMAIL_KEY, 'accounting@welld.ch'), 403)
  })

  it('throws ApiError on 404 (unknown key)', async () => {
    vi.mocked(apiFetch).mockResolvedValueOnce(problemResponse(404, 'Not Found', 'Unknown setting key'))
    await expectApiError(() => putSetting('not-a-real-key', 'accounting@welld.ch'), 404)
  })

  it('throws ApiError on 401 (no session)', async () => {
    vi.mocked(apiFetch).mockResolvedValueOnce(problemResponse(401, 'Unauthorized'))
    await expectApiError(() => putSetting(ACCOUNTING_DISTRIBUTION_EMAIL_KEY, 'accounting@welld.ch'), 401)
  })
  ...
  it('a non-Problem-JSON error body still produces an ApiError from the raw status', async () => {
    ...
    const err = await expectApiError(() => putSetting(ACCOUNTING_DISTRIBUTION_EMAIL_KEY, 'accounting@welld.ch'), 500)
