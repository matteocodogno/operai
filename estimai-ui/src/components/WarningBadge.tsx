...
export default function WarningBadge({ code, className = '' }: Props) {
  ...
  const open = useCallback((el: HTMLElement) => {
    ...
    setAnchor(onLeft ? { top: r.top, left: r.left } : { top: r.top, right: window.innerWidth - r.right })
  }, [])
      ...
      {anchor &&
        createPortal(
          ...
          >
            ...
            <span className="block text-[10.5px] text-soft leading-relaxed">{m.description}</span>
          </span>,
          document.body,
        )}
    </span>
