...
export default function MetricsBar({ totals, activityCount, releaseCount, profileCount }: MetricsBarProps) {
  return (
    <div className="bg-ink-soft border-b border-rule py-2.5 px-[22px] flex gap-2.5 flex-wrap">
      ...
      <Metric label="Confidence Range"    val={`${totals.best}–${totals.worst}`}   sub="days (best – worst case)"                                          colorClass="text-org" />
      <Metric label="AI Cost (total)"     val={totals.aiCost.toLocaleString()}     sub="coeff × FTE × elapsed days"                                        colorClass="text-[#b47fff]" />
      <Metric label="AI-assisted Elapsed" val={`${totals.aiElapsed}d`}             sub={`vs ${totals.el}d standard (−${totals.el - totals.aiElapsed}d)`}   colorClass="text-[#b47fff]" />
      <Metric label="Activities"          val={activityCount}                      sub={`${releaseCount} releases · ${profileCount} profiles`}              colorClass="text-soft" />
    </div>
  );
}
