...
- [ ] T1: auth — declare `settings` resource (read/manage) + seed grants — refs: AC-3.1 — deps: none  **[backend-dev]**
...
- [ ] T2: refund-api — `RefundSetting` schema + additive migration — refs: AC-5.2, US-3 — deps: none  **[backend-dev]**
...
- [ ] T3: refund-api — `settings` module (routes/service/repo/schemas + descriptor registry) — refs: AC-1.1, AC-1.2, AC-1.3, AC-1.4, AC-1.5, AC-3.1, AC-5.1, AC-5.3, AC-5.4 — deps: T2  **[backend-dev]**
...
- [ ] T4: refund-api — live recipient resolution + blocked-send + env removal — refs: AC-1.5, AC-2.1, AC-2.2, AC-2.3, AC-2.4, AC-2.5, AC-4.1, AC-4.3 — deps: T2, T3  **[backend-dev]**
...
- [ ] T5: refund-api — cutover seed script + runbook — refs: AC-4.2 — deps: T2, T3  **[backend-dev / devops]**
...
- [ ] T6: admin-ui — Refund-tab distribution-email panel + `settingsApi.ts` — refs: AC-1.1, AC-3.2, AC-5.3 — deps: T3 (contract; mock in tests)  **[frontend-dev]**
...
- [ ] T7: refund-ui — map `blocked_unconfigured` delivery status — refs: AC-2.2 (UI) — deps: none (contract; mock)  **[frontend-dev]**
