...
- [ ] T2: `POST /authz/app-access-check` — the eligibility **decision** endpoint — refs: AC-1.1, AC-1.2 — deps: T1
...
- [ ] T6: `resolveAccess` + widen every owner-scoped query + denial taxonomy — refs: AC-1.6, AC-2.1, AC-2.2, AC-2.3, AC-3.3, AC-5.2, AC-10.2, AC-10.3, AC-10.4 — deps: T4, T5
...
- [ ] T7: Optimistic concurrency — `version` CAS, required `If-Match`, `ETag`, 409/428 — refs: AC-4.1, AC-4.3, AC-4.4, AC-3.1, AC-3.2 — deps: T6
...
- [ ] T9: `PATCH` / `DELETE {collaboratorId}` / `DELETE /me` — manage, revoke, leave — refs: AC-5.1, AC-5.2, AC-5.3, AC-6.1, AC-6.2 — deps: T8
...
- [ ] T15: API clients — `collaboratorsApi`, `estimatesApi` `If-Match`/`ConflictError` — refs: AC-4.1, AC-1.2 — deps: T14
...
- [ ] T16: Context — `access` / `canEdit` / `version`, autosave gating and suppression — refs: AC-3.1, AC-3.2, AC-4.2 — deps: T15
  - touch: `estimai-ui/src/context/EstimatorContext.tsx`, `estimai-ui/src/pages/EstimatePage.tsx`
...
- [ ] T17: Viewer read-only gating across the editor — refs: AC-3.1, AC-3.2 — deps: T16
...
- [ ] T18: `ConflictBanner` + the fourth save-status state — refs: AC-4.1, AC-4.2 — deps: T16
...
- [ ] T19: Generalize `ConfirmDeleteModal` for Remove / Leave — refs: AC-5.2, AC-6.1 — deps: T14
...
- [ ] T22: Toolbar composition — "Share link" relabel + "Collaborators" entry / member chip — refs: AC-8.1, AC-8.2 — deps: T20, T21
  - touch: `estimai-ui/src/EstimatorApp.tsx`
...
- [ ] T23: Estimates list — shared indicator, owner identity, orphaned rows — refs: AC-2.1, AC-2.2, AC-2.3, AC-10.5, AC-10.4 — deps: T14, T15
  - touch: `estimai-ui/src/pages/EstimatesPage.tsx`
...
- [ ] T13: DevOps — environment wiring across all environments + single-instance guard — refs: AC-1.2, AC-7.1 — deps: T5, T1
