...
- [ ] T1: Add the `EmployeeAddress` model and its migration — refs: AC-1.2, AC-1.4, AC-2.5, AC-3.4 — deps: none
...
- [ ] T2: Add the shared zod schema and the pure formatted-address deriver — refs: AC-1.1, AC-1.4 — deps: none
...
- [ ] T3: Implement `GET`/`PUT /admin/users/{id}/address` — refs: AC-1.1, AC-1.2, AC-1.3, AC-1.4, AC-2.4, AC-2.5, AC-3.3, AC-3.4, AC-4.1, AC-5.1, AC-5.4, AC-6.3, AC-6.4 — deps: T1, T2
...
- [ ] T4: Implement `GET /me/address` (employee self-read) — refs: AC-6.1, AC-6.2, AC-6.3, AC-6.4 — deps: T1, T2
...
- [ ] T5: Add optional `targetType`/`targetId`/`action` filters to `GET /admin/audit` — refs: AC-5.3 — deps: none
...
- [ ] T6: Add the AC-5.2 application-level audit-immutability contract test — refs: AC-5.2 — deps: none
...
- [ ] T8: Extract the coordinate-staleness rule as a pure function — refs: AC-2.6, AC-3.4 — deps: none
...
- [ ] T9: Add the typed address API client — refs: AC-1.1, AC-1.2, AC-1.3, AC-5.3 — deps: T3, T5
...
- [ ] T10: Build the accessible combobox primitive — refs: AC-2.1, AC-2.2 — deps: none
...
- [ ] T11: Build `AddressSection` — the editor, its states, and the history panel — refs: AC-1.1, AC-1.3, AC-2.2, AC-2.3, AC-2.6, AC-3.1, AC-3.2, AC-3.3, AC-5.3 — deps: T7, T8, T9, T10, T13
...
- [ ] T12: Mount the section on `UserDetail` with capability-gated **absence** — refs: AC-4.2 — deps: T11
...
- [ ] T13: Add IT/EN copy constants for every new string — refs: AC-1.1, AC-1.4, AC-3.1 — deps: none
...
- [ ] T14: Build the read-only `AccountScreen` and its client — refs: AC-6.1, AC-6.2 — deps: T4
...
- [ ] T15: Add the ungated `/account` route and the `UserMenu` entry — refs: AC-6.1 — deps: T14
...
- [ ] T18: Record the release intent — refs: none (repo convention, not scope creep — CLAUDE.md mandates a changeset on app-code changes) — deps: T3, T4, T5, T6, T12, T15
